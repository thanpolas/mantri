 ...
 *   with string values, src can be an array to accomodate for grunt format.
 * @param  {Object}   options A map with options as defined in the grunt task.
 * @param  {string}   target  The target we are working on.
 */
mantri.run = function( cb, fileObj, options, target ) {

  // be defensive with source
  var srcRaw;
  if (Array.isArray(fileObj.src)) {
    srcRaw = fileObj.src[0];
  } else {
    srcRaw = fileObj.src;
  }
  ...
  // the Grunfile is.
  var documentRoot = options.root || srcRaw || './';
