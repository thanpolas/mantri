...
exports.usage = function() {
  ...
  grunt.log.writeln(' ' + path.basename(process.argv[1]) + ' [command] [mantri conf] [options]');
};
...
exports.pathOption = function() {
  grunt.log.header('Mantri Conf Option');

  grunt.log.writeln().writelns('The [mantri conf] option is the path to the ' +
    '"mantriConf.json" file, by default the cwd will be used to locate the conf file.');
};
...
exports.commands = function() {
  ...
  exports.table([
    ['init', 'Copy the mantri.web.js and mantriConf.json files to the folder' +
      ' you define. The init command treats the [mantri conf] option as the ' +
      ' destination folder. If no folder is defined the files will be copied to' +
...
exports.usageInit = function() {
  ...
  grunt.log.header('Usage');
  grunt.log.writeln(' ' + path.basename(process.argv[1]) + ' init [dest folder]' +
    ' --dest=destfolder/');
};
exports.usageDeps = function() {
  ...
  grunt.log.header('Usage');
  grunt.log.writeln(' ' + path.basename(process.argv[1]) + ' deps [mantri conf]' +
...
exports.usageBuild = function() {
  ...
  grunt.log.header('Usage');
  grunt.log.writeln(' ' + path.basename(process.argv[1]) + ' build [mantri conf]' +
