...
buildModule._checkModuleOutputWrapper = function(namespace, moduleConf) {
  ...
  return buildModule._getModuleOutputWrapper(namespace);
};

/**
 * Generate and return a custom output wrapper for the specific module.
 *
 * For a namespace of 'app.ctrl.dong' the following string would be returned
 * (without the newlines or spaces):
 *    this.app = app || {};
 *    this.app.ctrl = app.ctrl || {};
 *    this.app.ctrl.dong = app.ctrl.dong;
 *
 * @return {string} the proper js text that exports the module.
 * @private
 */
buildModule._getModuleOutputWrapper = function(namespace) {
  ...
  nsParts.forEach(function(part, index) {
    if (index === 0) {
      tempString = part;
      exportString = exportTarget+ '.' + tempString + '=' + tempString + orAssign;
    } else {
      tempString += '.' + part;
