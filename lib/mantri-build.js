...
build.run = function( opts, cb) {
  ...
  if ( !grunt.file.isFile(confFile) ) {
    helpers.log.error('No mantriConf.json file found: '.red + confFile + confFile.blue);
    cb( false );
    return;
  } else {
  ...
  var options = {
    target: opts.target,
    webConfig: webConfig,
    dest: opts.dest || webConfig.build.dest,
    cb: cb
  };
  ...
  var commands = [ {cmd: command, dest: opts.target} ];
