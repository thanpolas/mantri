...
build.mantriConfValidate = function(mantriConf, buildOpts, confFile) {
  ...
  if (__.isArray(mantriConf.buildModules)) {
    try {
      mantriConf.buildModules.forEach(build._validateModule);
    } catch(ex) {
      return false;
    }
  } else {
 ...
 *
 * @param {Object} buildModule The buildModule Object.
 * @private
 * @throws {Error} If Not valid, log message generated.
 */
build._validateModule = function(buildModule) {
  if ( !__.isString( buildModule.namespace ) ) {
    helpers.log.error('No "namespace" key found in an item of the "buildModules" ' +
