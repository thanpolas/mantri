...
build.run = function( opts, cb) {
  ...
  //
  //
  // Read mantriConf.json and find destination.
  //
  //
  ...
  // check for outputWrapper
  if (options.webConfig.build.outputWrapper) {
    options.outputWrapper = options.webConfig.build.outputWrapper;
  }
  ...
  helpers.log.debug( options.debug, 'Passed Validations.');
...
build._finishBuild = function( options, status ) {
  ...
  //
  //
  if (options.outputWrapper) {
    var bundledApp = grunt.file.read( options.dest );
    ...
    var wrappedApp = __.template(options.outputWrapper)({
      output: bundledApp
    }) + '\n'; // add a trailing newline
    grunt.file.write(options.dest, wrappedApp);
  ...
  helpers.log.info('\nBuild complete!'.green + ' Stats for: ' + options.dest.blue);
};
