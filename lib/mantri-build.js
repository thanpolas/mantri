...
build.useMantriConf = function(opts, cb) {
  ...
  //
  var buildOpts = {
    input: path.dirname(confFile) + '/' + mantriConf.build.input,
 ...
 *   input {string} The application bootstrap file.
...
build.run = function(buildOpts, cb) {
  ...
  //
  //
  var rootPath = [ buildOpts.documentRoot ];
  rootPath.push( options._tmpDir.path );
  console.log('buildOpts:', buildOpts);
  var cToolsOptions = {
    builder: helpers.getPath( CLOSURE_BUILDER ),
    inputs: path.dirname( options.confFile ) + '/' + buildOpts.input,
    compile: true,
