 ...
 *
 * @param  {string=} optConfPath The path to mantri conf.
 * @return {Object} Always an object, defined as defObj.
 */
function getConfOpts(optConfPath) {
  var confPath = optConfPath || 'mantriConf.json';
...
var cli = module.exports = function() {
  ...
  switch( task ) {
    case 'deps':
      ...
      opts = {
        src: parsedOpts.src || confOpts.deps.src,
        dest: parsedOpts.dest || confOpts.deps.dest,
        root: parsedOpts.root || confOpts.deps.root,
    ...
    case 'build':
      ...
      opts = {
        src: parsedOpts.src || confOpts.build.src,
        dest: parsedOpts.dest || confOpts.build.dest,
        debug: cli.options.debug,
        target: 'cli'
      };
    ...
    case 'init':
      ...
      // --dest defined path or second part of command
      parsedOpts.dest = parsedOpts.dest || confPath;
